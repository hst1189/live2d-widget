...
function showMessage(a, c, e) {
	(e || "" === sessionStorage.getItem("waifu-text") || null === sessionStorage.getItem(
		"waifu-text")) && (Array.isArray(a) && (a = a[Math.floor(Math.random() * a.length +
			1) - 1]), live2d_settings.showF12Message && console.log("[Message]", a.replace(
			/<[^<>]+>/g, "")), e && sessionStorage.setItem("waifu-text", a), $(
			".waifu-tips").stop(), $(".waifu-tips").html(a).fadeTo(200, 1), void 0 === c &&
		(c = 5E3), hideMessage(c))
}
...
function initModel(a, c) {
	...
	console.log(" ");
	"function" != typeof $.ajax && ("function" == typeof jQuery.ajax ? window.$ =
		jQuery : console.log("[Error] JQuery is not defined."));
	...
	"left" == live2d_settings.waifuEdgeSide[0] ? $(".waifu").css("left",
		live2d_settings.waifuEdgeSide[1] + "px") : "right" == live2d_settings.waifuEdgeSide[
		0] && $(".waifu").css("right", live2d_settings.waifuEdgeSide[1] + "px");
	window.waifuResize = function() {
		$(window).width() <= Number(live2d_settings.waifuMinWidth.replace("px", "")) ?
			$(".waifu").hide() : $(".waifu").show()
	};
	"disable" != live2d_settings.waifuMinWidth && (waifuResize(), $(window).resize(
		function() {
			waifuResize()
		}));
	try {
	...
	"auto" == live2d_settings.homePageUrl ? (window.location.protocol, window.location
		.hostname) : live2d_settings.homePageUrl;
	"file:" == window.location.protocol && "//" == live2d_settings.modelAPI.substr(
		0, 2) && (live2d_settings.modelAPI = "http:" + live2d_settings.modelAPI);
	...
	"object" == (void 0 === a ? "undefined" : _typeof(a)) ? loadTipsMessage(a): $.ajax({
		cache: !0,
		url: "" == a ? live2d_settings.tipsMessage : "waifu-tips.json" == a.substr(
			a.length - 15) ? a : a + "waifu-tips.json",
		dataType: "json",
		success: function(a) {
			loadTipsMessage(a)
		}
	...
	live2d_settings.modelStorage && null != a || (a = live2d_settings.modelId, c =
...
function loadModel(a) {
	...
	live2d_settings.modelStorage ? (localStorage.setItem("modelId", a),
		localStorage.setItem("modelTexturesId", c)) : (sessionStorage.setItem(
		"modelId", a), sessionStorage.setItem("modelTexturesId", c));
	loadlive2d("live2d", live2d_settings.modelAPI + "get/?id=" + a + "-" + c,
		live2d_settings.showF12Status ? console.log("[Status]", "live2d", "模型", a +
			"-" + c, "加载完成") : null)
}
...
function loadTipsMessage(a) {
	...
	function e() {
		...
		$.ajax({
			...
			dataType: "json",
			success: function(l) {
				1 != l.textures.id || 1 != b && 0 != b ? showMessage(f.load_rand_textures[
					1], 3E3, !0) : showMessage(f.load_rand_textures[0], 3E3, !0);
	...
	function g(a) {
		return live2d_settings.modelStorage ? localStorage.getItem(a) :
	...
	function k() {
		switch (live2d_settings.hitokotoAPI) {
			case "lwl12.com":
				...
				break;
			case "fghrsh.net":
				$.getJSON("https://api.fghrsh.net/hitokoto/rand/?encode=jsc&uid=3335",
					function(a) {
						if (!empty(a.source)) {
	...
	$.each(a.seasons, function(a, b) {
		...
		var d = b.date.split("-")[0],
			c = b.date.split("-")[1] || d;
		d.split("/")[0] <= a.getMonth() + 1 && a.getMonth() + 1 <= c.split("/")[0] &&
			d.split("/")[1] <= a.getDate() && a.getDate() <= c.split("/")[1] && (b =
				getRandText(b.text), b = b.render({
					year: a.getFullYear()
				}), showMessage(b, 6E3, !0))
	});
	...
	window.showWelcomeMessage = function(a) {
		if (window.location.href == live2d_settings.homePageUrl) {
			...
			b = getRandText(23 < b || 5 >= b ? a.waifu.hour_tips.t23 - 5 : 5 < b && 7 >=
				b ? a.waifu.hour_tips.t5 - 7 : 7 < b && 11 >= b ? a.waifu.hour_tips.t7 -
				11 : 11 < b && 14 >= b ? a.waifu.hour_tips.t11 - 14 : 14 < b && 17 >= b ?
				a.waifu.hour_tips.t14 - 17 : 17 < b && 19 >= b ? a.waifu.hour_tips.t17 -
				19 : 19 < b && 21 >= b ? a.waifu.hour_tips.t19 - 21 : 21 < b && 23 >= b ?
			...
			if ("" !== document.referrer) {
				...
				window.location.hostname == c.hostname ? b = d.localhost[0] + document.title
					.split(d.localhost[2])[0] + d.localhost[1] : "baidu" == e ? b = d.baidu[0] +
					c.search.split("&wd=")[1].split("&")[0] + d.baidu[1] : "so" == e ? b = d.so[
						0] + c.search.split("&q=")[1].split("&")[0] + d.so[1] : "google" == e ?
					b = d.google[0] + document.title.split(d.google[2])[0] + d.google[1] : ($
	...
	var f = a.waifu;
	live2d_settings.showHitokoto && (window.getActed = !1, window.hitokotoTimer =
		0, window.hitokotoInterval = !1, $(document).mousemove(function(a) {
			getActed = !0
		}).keydown(function() {
			getActed = !0
		}), setInterval(function() {
			getActed ? (getActed = hitokotoInterval = !1, window.clearInterval(
				hitokotoTimer)) : hitokotoInterval || (hitokotoInterval = !0,
